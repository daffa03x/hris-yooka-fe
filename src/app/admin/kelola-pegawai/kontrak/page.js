...
import { deletePegawaiKontrak, getPegawaiKontrak } from "@/app/services/pegawaiKontrak";
...
const KontrakPage = () => {
  const { data, searchQuery, setSearchQuery, pagination, loading, handleSearch, handleResetSearch, handlePageChange, handlePerPageChange, handleDelete } = useGetData(getPegawaiKontrak, deletePegawaiKontrak);

  return (
    <Dashboard>
      <div className="content">
        <h2 className="intro-y text-lg font-medium mt-10">Data Kontrak Pegawai</h2>
        <div className="grid grid-cols-12 gap-6 mt-5">
          <div className="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <LinkButton classNameLink="btn btn-primary" classNameLoading="btn btn-primary" link="/admin/kelola-pegawai/kontrak/create" title="Tambah Kontrak"/>
            ...
            <table className="table table-report w-full">
              <thead>
                <tr>
                  ...
                  <th className="whitespace-nowrap">Masa Bekerja</th>
                ...
                {loading ? (
                  ...
                  data.map((item, index) => (
                    <tr className="intro-x" key={index}>
                      ...
                      <td className="whitespace-nowrap">{sumMasaKerja(item.tanggal_mulai, Date.now())}</td>
                      <td className="whitespace-nowrap">{item.tanggal_mulai}</td>
                      <td className="whitespace-nowrap">{item.tanggal_akhir}</td>
                      <td className="table-report__action w-56">
                        <div className="flex justify-center items-center">
                          <EditButton link={`/admin/kelola-pegawai/kontrak/edit/${item.id}`} />
