...
import { deletePegawaiPendidikan, getPegawaiPendidikan } from "@/app/services/pegawaiPendidikan";
import Link from "next/link";
import { FaCircleNotch, FaMagnifyingGlass, FaPenToSquare, FaRegTrashCan, FaX } from "react-icons/fa6";
...
const PendidikanPage = () => {
  const { data, searchQuery, setSearchQuery, pagination, loading, handleSearch, handleResetSearch, handlePageChange, handlePerPageChange, handleDelete } = useGetData(getPegawaiPendidikan, deletePegawaiPendidikan);

  return (
    <Dashboard>
      <div className="content">
        <h2 className="intro-y text-lg font-medium mt-10">Data Pendidikan Pegawai</h2>
        <div className="grid grid-cols-12 gap-6 mt-5">
          <div className="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <LinkButton classNameLink="btn btn-primary" classNameLoading="btn btn-primary" link="/admin/kelola-pegawai/pendidikan/create" title="Tambah Pendidikan"/>
            ...
            <table className="table table-report w-full">
              <thead>
                <tr>
                  ...
                  <th className="whitespace-nowrap">Instansi</th>
                  <th className="whitespace-nowrap">Jurusan</th>
                  <th className="whitespace-nowrap">Jenjang</th>
                  <th className="whitespace-nowrap">Ijazah</th>
                ...
                {loading ? (
                  ...
                  data.map((item, index) => (
                    <tr className="intro-x" key={index}>
                      ...
                      <td className="whitespace-nowrap">{item.nama_instansi}</td>
                      <td>{item.jurusan}</td>
                      <td>{item.jenjang}</td>
                      ...
                      <td className="table-report__action w-56">
                        <div className="flex justify-center items-center">
                          <EditButton link={`/admin/kelola-pegawai/pendidikan/edit/${item.id}`} />
