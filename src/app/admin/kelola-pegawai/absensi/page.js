...
import { deletePegawaiAbsensi, getPegawaiAbsensi } from "@/app/services/pegawaiAbsensi";
...
const Absensi = () => {
  const { 
    ...
    getPegawaiAbsensi,
    deletePegawaiAbsensi
  );

  return (
    <Dashboard>
      <div className="content">
        <h2 className="intro-y text-lg font-medium mt-10">Data Absensi Pegawai</h2>
        <div className="grid grid-cols-12 gap-6 mt-5">
          <div className="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <LinkButton classNameLink="btn btn-primary" classNameLoading="btn btn-primary" link="/admin/kelola-pegawai/absensi/create" title="Tambah Absensi"/>
            ...
            <table className="table table-report w-full">
              <thead>
                <tr>
                  ...
                  <th className="whitespace-nowrap">Jam Masuk</th>
                  <th className="whitespace-nowrap">Jam Pulang</th>
                ...
                {loading ? (
                  ...
                  data.map((item, index) => (
                    <tr className="intro-x" key={index}>
                      ...
                      <td className="whitespace-nowrap">{item.tanggal_absen}</td>
                      <td className="whitespace-nowrap">{item.jam_masuk}</td>
                      <td className="whitespace-nowrap">{item.jam_pulang}</td>
                      <td className="whitespace-nowrap">{item.status_absen}</td>
                      <td className="table-report__action w-56">
                        <div className="flex justify-center items-center">
                          <EditButton link={`/admin/kelola-pegawai/absensi/edit/${item.id}`} />
